...
public:
  json prep(const json &shared) override { return shared["input_data"]; }

  json exec(const json &prep_res) override {
    return json{{"processed", true}, {"original", prep_res}};
  }

  json post(const json &shared, const json &prep_res,
            const json &exec_res) override {
...
public:
  json prep(const json &shared) override {
    return json{{"query", shared["query"]},
  ...
  json post(const json &shared, const json &prep_res,
    ...
    const_cast<json &>(shared)["iteration"] =
        prep_res.value("iteration", 0) + 1;
...
public:
  json prep(const json &shared) override { return shared["query"]; }

  json exec(const json &prep_res) override {
    return json{
        {"results", "Search results for: " + prep_res.get<std::string>()}};
  }
...
public:
  ...
  json exec(const json &prep_res) override {
    return json{{"answer",
                 "Final answer for: " + prep_res["query"].get<std::string>()}};
  }
...
TEST(APICompatibilityTest, SharedStateBehavior) {
  json shared = {{"input_data", json{{"value", 42}}},
...
TEST(APICompatibilityTest, FlowOrchestrationBranching) {
  ...
  EXPECT_TRUE(shared1["final_answer"]["answer"].get<std::string>().find(
                  "What is 2+2?") != std::string::npos);
...
TEST(APICompatibilityTest, BatchProcessing) {
  ...
  public:
    json prep(const json &shared) override { return shared["batch_data"]; }
  ...
  json shared = {{"batch_data", json::array({"item1", "item2", "item3"})}};
  ...
  EXPECT_EQ(batch_results[0]["processed_item"].get<std::string>(),
            "item1_processed");
  EXPECT_EQ(batch_results[1]["processed_item"].get<std::string>(),
            "item2_processed");
  EXPECT_EQ(batch_results[2]["processed_item"].get<std::string>(),
            "item3_processed");
}
...
TEST(APICompatibilityTest, ParameterManagement) {
  ...
  json params = {{"setting1", "value1"}, {"setting2", 42}, {"setting3", true}};
...
TEST(APICompatibilityTest, RetryLogic) {
  ...
  public:
    ...
    json post(const json &shared, const json &prep_res,
              const json &exec_res) override {
      const_cast<json &>(shared)["result"] = exec_res;
      return "success"; // Return action string
    }

    json exec_fallback(const json &prep_res,
  ...
  // Verify retry behavior worked - should return action string
  EXPECT_TRUE(result.is_string());
  EXPECT_EQ(result.get<std::string>(), "success");

  // Verify shared state was updated
  EXPECT_TRUE(shared.contains("result"));
  EXPECT_TRUE(shared["result"]["success"].get<bool>());
  EXPECT_EQ(shared["result"]["attempts"].get<int>(), 3);
}
...
TEST(APICompatibilityTest, CompleteAPISurface) {
  ...
  // Create a proper BatchNode implementation for testing
  class TestBatchNode : public BatchNode {
  public:
    json prep(const json &shared) override {
      return shared.value("batch_data", json::array());
    }

    json exec(const json &item) override { return json{{"processed", item}}; }

    json post(const json &shared, const json &prep_res,
              const json &exec_res) override {
      const_cast<json &>(shared)["batch_result"] = exec_res;
      return "default";
    }
  };

  auto batch_node = std::make_shared<TestBatchNode>();
  ...
  EXPECT_TRUE(base_node->get_params().contains("test"));
  EXPECT_EQ(base_node->get_params()["test"].get<bool>(), true);

  // Test shared state with proper input data
  json shared = {{"input_data", json{{"value", 42}}}};
  json result = base_node->run(shared);
